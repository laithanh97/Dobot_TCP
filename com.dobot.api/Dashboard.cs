...
namespace Dobot_TCP.com.dobot.api
{
    class Dashboard : DobotClient
    {
        ...
        public string PowerOn()
        {
            if (!IsConnected()) return "device does not connected!";
            string str = "PowerOn()";
            return !SendData(str) ? str + ":send error" : WaitReply(15000);
        }
        ...
        public string EnableRobot()
        {
            ...
            return !SendData(str) ? str + ":send error" : WaitReply(20000);
        }
        ...
        {
            return !IsConnected() ? "device does not connected!!!" : (!SendData(str) ? str + ":send error" : WaitReply(5000));
        }
    }
}
